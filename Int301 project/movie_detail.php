...
while($row = mysql_fetch_array($result))
 {
  ...
  echo '<h1>'.$row['movieName'].'</h1>';
   echo ' <div id="mainpic" >';
       $mID=$row['movieID'];
       echo' <img  class="image" src="images/details/'.$row['movieID'].'.jpg" />';
       echo ' </div>';
    echo ' <div id="info">';
    ...
    $category = mysql_query("SELECT * FROM `category` WHERE `caID`IN(SELECT `caID` FROM `mcate`AS a LEFT JOIN `movies` as m ON a.`movieID` =m.`movieID` WHERE m.`movieID`=$mId)");
...
echo '<h2>COMMENTS</h2>';
  ...
  $comment = mysql_query("SELECT * FROM `comment` WHERE `movieID` = $mID");
